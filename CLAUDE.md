...
- Сохраняет предсказание направления (up/down) в `predict_path/{YYYY-MM-DD}.txt`
...
**Python-библиотеки:** `pandas`, `numpy`, `sqlite3`, `pickle`, `yaml`, `chromadb`, `tiktoken`, `matplotlib`, `requests`
...
- Логирование: каждый скрипт создаёт файл `{script_name}_{timestamp}.txt` в `rts/log/`, ротация до 3 файлов
