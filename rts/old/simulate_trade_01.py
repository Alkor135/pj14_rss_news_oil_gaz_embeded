...
def predict_next_session(
    ...
    best_k: int,
    top_k_chunks: int = 5,
    output_dir: Path = Path(r"C:\Users\Alkor\gd\predict_ai\rts_all_oil_gaz_gemma")
):
    """
    Делает предсказание направления (up/down) для СЛЕДУЮЩЕЙ торговой сессии
    на основе последней доступной даты в embeddings.

    Логика полностью повторяет compute_max_k:
    - берём последнюю дату
    - сравниваем с предыдущими best_k днями
    - находим самый похожий
    - смотрим знак NEXT_BODY у него
    - формируем прогноз
    - сохраняем txt файл
    """
    ...
    # --- объединяем как в основном расчёте ---
    df_combined = df_bar.join(df_emb[['CHUNKS']], how='inner')
    df_combined.sort_index(inplace=True)

    if len(df_combined) < best_k + 1:
        logging.warning("Недостаточно данных для предсказания")
        return

    # Последняя дата в embeddings
    last_date = df_combined.index[-1]
    last_pos = len(df_combined) - 1

    chunks_cur = df_combined.iloc[last_pos]["CHUNKS"]

    similarities = []
    indices = []

    # сравниваем с предыдущими best_k днями
    for j in range(last_pos - best_k, last_pos):
        chunks_prev = df_combined.iloc[j]["CHUNKS"]
    ...
    best_date = df_combined.index[best_j]

    # получаем explain-информацию
    score_explained, pairs = chunks_similarity_with_explain(
        chunks_cur,
        df_combined.iloc[best_j]["CHUNKS"],
        top_k=top_k_chunks
    )

    body_prev = df_combined.iloc[best_j]["NEXT_BODY"]

    # Определяем направление
    predicted_direction = "up" if body_prev > 0 else "down"

    # Формируем список дат для сравнения
    compare_dates = [
        df_combined.index[j].strftime("%Y-%m-%d")
        for j in indices
    ]

    # Имя файла = последняя дата embeddings
    filename = output_dir / f"{last_date.strftime('%Y-%m-%d')}.txt"

    # ---- запись файла ----
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            f"Дата предсказания: {last_date.strftime('%Y-%m-%d')}, "
            f"предыдущих дней для сравнения {best_k}:\n"
        )
        f.write(f"Даты для сравнения: {', '.join(compare_dates)}\n")
        f.write(f"Предсказанное направление: {predicted_direction}\n")
        f.write(f"Процент сходства: {best_score * 100:.2f}%\n")
        f.write("Метаданные ближайшего похожего:\n")
        f.write(f"  date: {best_date.strftime('%Y-%m-%d')}\n")

        # если в CHUNKS есть md5
        try:
            md5_val = df_combined.iloc[best_j]["CHUNKS"][0].get("md5", "N/A")
        except Exception:
            md5_val = "N/A"

        f.write(f"  md5: {md5_val}\n")

    logging.info(f"✅ Предсказание сохранено: {filename}")
    logging.info(
        f"🔎 Направление: {predicted_direction} | "
        f"Сходство: {best_score:.4f} | "
        f"Похожая дата: {best_date}"
    )
...
def main(path_db_day, cache_file):
    ...
    # ===============================
    # 🔮 Предсказание следующей сессии
    # ===============================

    # определяем лучший k по последней дате
    last_trade_date = df_rez.index[-1]
    best_k_for_prediction = int(df_rez.loc[last_trade_date]["max"])

    logging.info(f"🏆 Лучшее окно для предсказания: k={best_k_for_prediction}")

    predict_next_session(
        df_bar=df_bar,
        df_emb=df_emb,
        best_k=best_k_for_prediction
    )
